...
public class Table implements Iterable<Record>, Closeable {
  ...
  public BacktrackingIterator<RecordId> ridIterator() {
    List<RecordId> rids = getAllRecordIds();
    return new ArrayBacktrackingIterator<RecordId>(rids.toArray(new RecordId[rids.size()]));
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    ...
    RIDBlockIterator(BacktrackingIterator<Page> block) {
      this.block = block;
      throw new UnsupportedOperationException("TODO(hw3): implement");
     ...
     * 100 pages.
     */
    RIDBlockIterator(Iterator<Page> pageIter, int maxPages) {
   ...
   */
  private List<RecordId> getAllRecordIds() {
    Iterator<Page> pageIterator = Table.this.allocator.iterator();
    List<RecordId> res = new ArrayList<>();

    pageIterator.next();
    while (pageIterator.hasNext()) {
      Page page = pageIterator.next();
      for (short entryNum = 0; entryNum < Table.this.numRecordsPerPage && res.size() < numRecords; ++entryNum) {
